...
const currentStep = 6;
  ...
  const interests = [
    '운동', '산책', '공연관람', '쇼핑', '재태크', '패션', '반려동물', '음악감상', '독서',
    '여행', '카페', '게임', '영화/드라마', '전시관람', '연극/뮤지컬', '술', '악기연주', '맛집', '요리',
    '애니', '자기계발', '드라이브', '봉사활동', '요가/필라테스',
  ];
  ...
  return (
    <div className="home7">
      ...
      <div className="hobby-container">
        {interests.slice(0, 12).map((interest, index) => (
          <button
            key={index}
        ...
        {interests.slice(12).map((interest, index) => (
          <button
            key={index}
            className={`hobby-btn ${selected.includes(interest) ? 'selected' : ''}`}
            onClick={() => toggleSelect(interest)}
